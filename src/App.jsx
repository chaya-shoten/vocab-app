...
const extractWords = (input) => { const stopwords = new Set([ "the", "and", "was", "for", "are", "but", "not", "you", "all", "any", "can", "had", "her", "his", "how", "man", "our", "out", "say", "she", "too", "use", "a", "an", "by", "do", "if", "in", "into", "is", "it", "no", "of", "on", "or", "such", "that", "their", "then", "there", "these", "they", "this", "to", "with", "would", "as", "at", "be", "have", "from", "including", "what", "due", "see" ]); const matches = input.toLowerCase().match(/\b[a-z]{3,}\b/g) || []; return [...new Set(matches.filter(w => !stopwords.has(w)))]; };

const fetchWordData = async (word) => { try { const res = await fetch(/api/word-info?word=${encodeURIComponent(word)}); if (!res.ok) throw new Error('Network response was not ok'); return await res.json(); } catch (error) { console.error('Fetch word data error:', error); return { meaning: "取得エラー", synonyms: [], simpleSynonyms: [], etymology: "", culturalBackground: "", trivia: "", images: [] }; } };
