...
export default function App() {
  ...
  const extractWords = (text) => {
    const stopwords = new Set([
      ...
      "can", "had", "her", "his", "how", "man", "our", "out", "say", "she", "too",
      "use", "a", "an", "by", "do", "if", "in", "into", "is", "it", "no", "of", "on",
      "or", "such", "that", "their", "then", "there", "these", "they", "this", "to",
  ...
  const wordData = wordDataMap[currentWord];

  const handleResponse = (known) => {
    if (known) {
      const updated = [...new Set([...knownWords, currentWord])];
  ...
  return (
    <div style={{ padding: 20, maxWidth: 600, margin: "0 auto" }}>
      <h2 style={{ fontSize: 20, marginBottom: 10 }}>英文を入力してください</h2>
      <textarea
      ...
      <div style={{ marginTop: 10 }}>
        <button onClick={handleSubmitText} style={{ fontSize: 16 }}>単語抽出して学習開始</button>
        <button onClick={resetProgress} style={{ marginLeft: 10, fontSize: 16 }}>進捗をリセット</button>
      </div>

      {loading && <p style={{ marginTop: 20 }}>データ取得中...</p>}

      {words.length > 0 && currentIndex < words.length && !knownWords.includes(currentWord) && (
        <div style={{ marginTop: 20 }}>
          <p>進捗: {progress}%</p>
          <h3 style={{ fontSize: 22 }}>{currentWord}</h3>
          {!showDetails ? (
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => handleResponse(true)} style={{ fontSize: 16 }}>Yes</button>
              <button onClick={() => handleResponse(false)} style={{ fontSize: 16 }}>No</button>
            </div>
          ) : wordData ? (
            <div style={{ marginTop: 10 }}>
              ...
              <div style={{ display: "flex", gap: 10, overflowX: "scroll", marginTop: 10 }}>
                {wordData.images.map((img, i) => (
                  <img key={i} src={img} alt={`img-${i}`} style={{ height: 100 }} />
                ))}
              </div>
              <button onClick={nextWord} style={{ marginTop: 10, fontSize: 16 }}>次の単語へ</button>
            </div>
          ) : (
